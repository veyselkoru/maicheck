...
// ─── DATA BUILDER ────────────────────────────────────────────────────────────
async function buildReportData(auditId: string, companyId: string) {
...
// ─── HTML GENERATORS ─────────────────────────────────────────────────────────
function tlColor(tl: string) { return tl === 'green' ? '#22c55e' : tl === 'yellow' ? '#f59e0b' : '#ef4444'; }
function tlText(tl: string)  { return tl === 'green' ? 'DÜŞÜK RİSK'  : tl === 'yellow' ? 'ORTA RİSK' : 'YÜKSEK RİSK'; }
function rlColor(rl: string | null) { return rl === 'KRITIK' ? '#dc2626' : rl === 'YUKSEK' ? '#ea580c' : rl === 'ORTA' ? '#f59e0b' : '#22c55e'; }
function compLabel(c: string | null) { return c === 'UYGUN' ? 'Uygun' : c === 'KISMI' ? 'Kısmi' : c === 'UYGUN_DEGIL' ? 'Uygun Değil' : '—'; }
function rlLabel(rl: string | null) { return rl === 'KRITIK' ? 'Kritik' : rl === 'YUKSEK' ? 'Yüksek' : rl === 'ORTA' ? 'Orta' : rl === 'DUSUK' ? 'Düşük' : '—'; }
function fmtDate(d: Date | null | undefined) { if (!d) return '—'; return new Date(d).toLocaleDateString('tr-TR'); }
function pct(a: number, b: number) { if (b === 0) return '0'; return ((a/b)*100).toFixed(1); }

function compliancePct(stats: ReturnType<typeof calcAuditStats>) {
  const evaluated = stats.uygun + stats.kismi + stats.uygunDegil;
  if (evaluated === 0) return '—';
  return `%${pct(stats.uygun, evaluated)}`;
}

function genDetailedHtml(d: Awaited<ReturnType<typeof buildReportData>>, companyLogoPath: string | null, subLogoPath: string | null, userNotes: string[]): string {
...
<div class="cover">
  ...
  <div class="cover-meta">
    ...
    <div class="meta-item"><div class="meta-lbl">ŞABLON</div><div class="meta-val">${(audit.template as any)?.name || '—'}</div></div>
  </div>
</div>
...
function genExecHtml(d: Awaited<ReturnType<typeof buildReportData>>, companyLogoPath: string | null, subLogoPath: string | null, userNotes: string[]): string {
  ...
  const trend = ((): string => {
    if (stats.uygunDegil === 0) return 'Bu denetimde hiç uygunsuzluk tespit edilmemiştir. Mevcut uyum seviyesi korunmalıdır.';
    if (stats.kritikYuksek > 3) return `${stats.kritikYuksek} adet kritik/yüksek risk tespiti öne çıkmaktadır. Derhal müdahale gereklidir.`;
...
// ─── DOCX GENERATOR ──────────────────────────────────────────────────────────
async function generateDocx(d: Awaited<ReturnType<typeof buildReportData>>, reportType: 'DETAYLI' | 'YONETICI_OZETI', notes: string[]): Promise<string> {
  const { Document, Packer, Paragraph, Table, TableRow, TableCell, TextRun, HeadingLevel, AlignmentType, BorderStyle, WidthType, ShadingType } = await import('docx');
  const { audit, stats, areaStats, nonCompliant, kismi, tl } = d;
  const evaluated = stats.uygun + stats.kismi + stats.uygunDegil;
  const fmtD = (dt: Date | null | undefined) => dt ? new Date(dt).toLocaleDateString('tr-TR') : '—';
  const mkPara = (text: string, bold = false, size = 22, color?: string) => new Paragraph({ children: [new TextRun({ text, bold, size, color })] });
  const mkHead = (text: string, level: any = HeadingLevel.HEADING_2) => new Paragraph({ text, heading: level, spacing: { before: 240, after: 120 } });
  ...
  const headerRow = (cols: string[]) => new TableRow({ tableHeader: true, children: cols.map(c => new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: c, bold: true, size: 18, color: 'FFFFFF' })] })], shading: { type: ShadingType.SOLID, color: '1e40af' } })) });

  const sections: any[] = [];

  if (reportType === 'DETAYLI') {
    sections.push(
      new Paragraph({ text: 'DETAYLı DENETİM RAPORU', heading: HeadingLevel.TITLE, alignment: AlignmentType.CENTER }),
      mkPara(`${audit.company.name} — ${audit.subcontractor.name}`, true, 26),
      mkPara(`Dönem: ${audit.periodLabel} | Rapor Tarihi: ${fmtD(new Date())}`, false, 20, '64748b'),
      new Paragraph({ text: '', spacing: { after: 200 } }),
      mkHead('1. Taraf Bilgileri'),
      mkPara('Asıl İşveren:', true),
      infoTable([['Unvan', audit.company.name], ['Vergi No', audit.company.taxNumber || '—'], ['Adres', audit.company.address || '—']]),
      new Paragraph({ text: '', spacing: { after: 100 } }),
      mkPara('Alt İşveren:', true),
      infoTable([['Unvan', audit.subcontractor.name], ['Vergi No', audit.subcontractor.taxNumber || '—'], ['SGK Sicil No', audit.subcontractor.sgkSicilNo || '—'], ['Sorumlu', audit.subcontractor.responsiblePerson || '—']]),
      mkHead('2. Denetim Bilgileri'),
      infoTable([['Dönem', audit.periodLabel || '—'], ['Dönem Türü', audit.periodType === 'SINGLE' ? 'Tek Dönem' : 'Dönem Aralığı'], ['Denetçi', audit.auditorName || '—'], ['Şablon', (audit.template as any)?.name || '—'], ['Rapor Tarihi', fmtD(new Date())]]),
      mkHead('3. Özet İstatistikler'),
      infoTable([['Toplam Kontrol', String(stats.total)], ['Uygun', String(stats.uygun)], ['Kısmi Uygun', String(stats.kismi)], ['Uygun Değil', String(stats.uygunDegil)], ['Kritik/Yüksek', String(stats.kritikYuksek)], ['Eksik Belge', String(stats.docMissing)], ['Toplam Risk Puanı', String(stats.totalRisk)], ['Uygunluk Oranı', `${pct(stats.uygun, evaluated || 1)}%`]]),
      mkHead('4. Alan Bazlı Risk Tablosu'),
      new Table({
        rows: [
      ...
      mkHead('5. Uygun Olmayan Maddeler'),
      ...nonCompliant.length === 0 ? [mkPara('Bu denetimde uygun olmayan madde tespit edilmemiştir.', false, 20, '22c55e')] : [
        new Table({
          rows: [
      ...
      mkHead('6. Kısmi Uygun Maddeler'),
      ...kismi.length === 0 ? [mkPara('Kısmi uygun madde bulunmamaktadır.')] : [
      ...
      ...(notes.length > 0 ? [mkHead('7. Kullanıcı Notları'), ...notes.map(n => mkPara(`• ${n}`, false, 20))] : []),
      mkHead('8. İmza Alanları'),
      new Paragraph({ text: '', spacing: { after: 600 } }),
      mkPara('Denetçi: ____________________     Alt İşveren Yetkilisi: ____________________     Asıl İşveren Yetkilisi: ____________________'),
      new Paragraph({ text: '', spacing: { after: 200 } }),
      mkPara('mAicheck by mAiTechs Smart Solutions', false, 18, '94a3b8'),
    );
  } else {
    // EXEC SUMMARY
    const top10 = nonCompliant.sort((a,b)=>(b.riskScore??0)-(a.riskScore??0)).slice(0,10);
    sections.push(
      new Paragraph({ text: 'YÖNETİCİ ÖZETİ RAPORU', heading: HeadingLevel.TITLE, alignment: AlignmentType.CENTER }),
      mkPara(`${audit.subcontractor.name} — ${audit.periodLabel}`, true, 24),
      mkPara(`${audit.company.name} | ${fmtD(new Date())}`, false, 20, '64748b'),
      new Paragraph({ text: '', spacing: { after: 180 } }),
      mkHead('1. Denetimin Amacı'),
      mkPara(`${audit.company.name} bünyesinde hizmet veren ${audit.subcontractor.name} firması için ${audit.periodLabel} dönemi; iş hukuku, SGK, vergi ve İSG kapsamında denetlenmiştir. Toplam ${stats.total} kontrol maddesi değerlendirilmiştir.`),
      mkHead('2. Genel Risk Durumu'),
      infoTable([['Toplam Risk Puanı', String(stats.totalRisk)], ['Risk Seviyesi', tlText(tl)], ['Uygun Madde', String(stats.uygun)], ['Uygun Değil', String(stats.uygunDegil)], ['Kritik/Yüksek', String(stats.kritikYuksek)], ['Uygunluk Oranı', `${pct(stats.uygun, evaluated||1)}%`], ['Eksik Belge', String(stats.docMissing)]]),
      mkHead('3. En Kritik 10 Bulgu'),
      ...top10.length === 0 ? [mkPara('Kritik bulgu tespit edilmemiştir.', false, 20, '22c55e')] : [
      ...
      mkHead('4. Alan Bazlı Risk Dağılımı'),
      new Table({ rows: [headerRow(['Alan','Uygun Değil','Kritik/Yüksek','Risk Puanı']), ...areaStats.filter(a=>a.totalRisk>0).slice(0,15).map(a=>new TableRow({children:[new TableCell({children:[mkPara(a.area)]}),new TableCell({children:[mkPara(String(a.uygunDegil),true,20,a.uygunDegil>0?'ef4444':'22c55e')]}),new TableCell({children:[mkPara(String(a.kritikYuksek),false,20)]}),new TableCell({children:[mkPara(String(a.totalRisk),true)]})]}))] , width:{size:100,type:WidthType.PERCENTAGE} }),
      mkHead('5. Performans Değerlendirmesi'),
      mkPara(`${audit.subcontractor.name}: Uygunluk oranı ${pct(stats.uygun, evaluated||1)}%, toplam risk puanı ${stats.totalRisk}. Risk seviyesi: ${tlText(tl)}.`),
      mkHead('6. Kritik Aksiyon Önerileri'),
      ...nonCompliant.filter(i=>i.riskLevel==='KRITIK'||i.riskLevel==='YUKSEK').slice(0,8).map((i,n)=>mkPara(`${n+1}. [${i.auditArea}] ${i.correctiveActionText||i.controlSubject} — Sorumlu: ${i.responsiblePerson||'—'} — Son Tarih: ${fmtD(i.dueDate)}`)),
      mkHead('7. Sonuç ve Öneriler'),
      mkPara(`${audit.subcontractor.name} firması ${audit.periodLabel} dönemi denetiminde ${tlText(tl)} kategorisinde değerlendirilmiştir. ${stats.uygunDegil} uygunsuzluk tespit edilmiş olup düzeltici aksiyonlar tanımlanmıştır. Eksik ${stats.docMissing} belgenin ivedilikle tamamlanması önerilmektedir.`),
      ...(notes.length > 0 ? [mkHead('8. Denetçi Notları'), ...notes.map(n => mkPara(`• ${n}`, false, 20))] : []),
...
router.get('/audit/:auditId', async (req: AuthRequest, res) => {
  ...
  if (!audit) throw createError('Denetim bulunamadı', 404);
  const reports = await prisma.report.findMany({ where: { auditId: req.params.auditId, companyId: req.user!.companyId }, include: { generatedBy: { select: { name: true } }, versions: { orderBy: { versionNo: 'desc' }, take: 5, include: { createdBy: { select: { name: true } } } }, notes: { include: { user: { select: { name: true } } } } }, orderBy: { generatedAt: 'desc' } });
  res.json(reports);
});

// POST /api/reports/generate — build HTML + DOCX
router.post('/generate', async (req: AuthRequest, res) => {
  const schema = z.object({ auditId: z.string().uuid(), reportType: z.enum(['DETAYLI','YONETICI_OZETI']), notes: z.array(z.string()).optional() });
  ...
  try { docxPath = await generateDocx(d, reportType, notes); } catch (err) { console.warn('DOCX gen failed:', (err as Error).message); }

  // Get or create report record
  let report = await prisma.report.findFirst({ where: { auditId, companyId: req.user!.companyId, reportType: reportType as any } });
  if (!report) {
    report = await prisma.report.create({ data: { companyId: req.user!.companyId, auditId, reportType: reportType as any, title: `${reportType === 'DETAYLI' ? 'Detaylı Denetim Raporu' : 'Yönetici Özeti'} — ${d.audit.periodLabel}`, status: 'YAYINLANDI', generatedById: req.user!.id } });
  } else {
    await prisma.report.update({ where: { id: report.id }, data: { updatedAt: new Date() } });
  }

  const lastVersion = await prisma.reportVersion.findFirst({ where: { reportId: report.id }, orderBy: { versionNo: 'desc' } });
  const versionNo = (lastVersion?.versionNo ?? 0) + 1;
  const version = await prisma.reportVersion.create({ data: { reportId: report.id, versionNo, systemContentJson: { stats: d.stats, areaStats: d.areaStats, nonCompliantCount: d.nonCompliant.length } as any, htmlContent, docxPath, createdById: req.user!.id } });

  if (notes.length > 0) {
    await Promise.all(notes.map(n => prisma.reportUserNote.create({ data: { reportId: report!.id, reportVersionId: version.id, userId: req.user!.id, noteContent: n } })));
  }
...
router.get('/:id/html', async (req: AuthRequest, res) => {
  await assertTenant('report', req.params.id, req.user!.companyId);
  const version = await prisma.reportVersion.findFirst({ where: { reportId: req.params.id }, orderBy: { versionNo: 'desc' } });
  if (!version?.htmlContent) { res.status(404).json({ message: 'HTML içerik bulunamadı. Raporu yeniden oluşturun.' }); return; }
...
router.get('/:id/docx', async (req: AuthRequest, res) => {
  await assertTenant('report', req.params.id, req.user!.companyId);
  const version = await prisma.reportVersion.findFirst({ where: { reportId: req.params.id }, orderBy: { versionNo: 'desc' } });
  if (!version?.docxPath) { res.status(404).json({ message: 'DOCX bulunamadı. Raporu yeniden oluşturun.' }); return; }
  const filepath = path.join(process.cwd(), version.docxPath);
  if (!fs.existsSync(filepath)) { res.status(404).json({ message: 'DOCX dosyası sunucuda bulunamadı.' }); return; }
  res.download(filepath, `maicheck_report_v${version.versionNo}.docx`);
});

// GET /api/reports/:id/version/:vid/html
router.get('/:id/version/:vid/html', async (req: AuthRequest, res) => {
  await assertTenant('report', req.params.id, req.user!.companyId);
  const version = await prisma.reportVersion.findFirst({ where: { id: req.params.vid, reportId: req.params.id } });
  if (!version?.htmlContent) { res.status(404).json({ message: 'Bu versiyon için HTML bulunamadı.' }); return; }
  res.setHeader('Content-Type', 'text/html; charset=utf-8');
  res.send(version.htmlContent);
});
...
router.post('/:id/notes', async (req: AuthRequest, res) => {
  await assertTenant('report', req.params.id, req.user!.companyId);
  const schema = z.object({ noteTitle: z.string().optional(), noteContent: z.string().min(1) });
  const data = schema.parse(req.body);
  const note = await prisma.reportUserNote.create({ data: { reportId: req.params.id, userId: req.user!.id, ...data } as any });
  res.status(201).json(note);
});

router.get('/', async (req: AuthRequest, res) => {
  const reports = await prisma.report.findMany({ where: { companyId: req.user!.companyId }, include: { audit: { select: { periodLabel:true, subcontractor:{ select:{ name:true } } } }, generatedBy: { select: { name:true } }, versions: { orderBy: { versionNo: 'desc' }, take: 1 }, _count: { select: { notes:true } } }, orderBy: { generatedAt: 'desc' }, take: 50 });
  res.json(reports);
});
